...
% This script is for estimating a GLM specified using the SpecifyModel. 
% Allows user to display the trial type onsets/durations in the SPM Batch
% GUI.
%
...
Analysis.dir  = fullfile('/path/to/analysis/output', Analysis.name);

Func.dir         = '/path/to/preprocessing/output/directory';
Func.wildcard    = '^swa.*\.nii';
Func.motwildcard = 'rp_.*\.txt'; %removed ^ from start
...
for curSub = 1:length(subjects)
    ...
    SpecModelMats   = cellstr(spm_select('List', Model.directory, '.*\.mat'));
    ...
    try
        %matlabbatch = setModelparams(Model,show,mask)
    ...
    mkdir(fullfile(Analysis.dir, subjects{curSub}));
    save(fullfile(Analysis.dir, subjects{curSub}, 'Job.mat'), 'matlabbatch');
